  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="display text-4xl text-ink sm:text-5xl">{team.name}</h1>
          <p className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-ink-muted">
            ...
            <span>
              <span className="tnum">{players.length}</span> on the roster
            </span>
            <span aria-hidden className="text-ink-subtle">·</span>
            {/* Coaches lost its top-level tab, so it lives where a coach looks
                for it: on the team. */}
            <Link href="/coaches" className="ring-focus rounded text-accent hover:underline">
              Coaches
            </Link>
          </p>
        </div>
